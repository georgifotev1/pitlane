...
// Line values are held as strings (so an empty field shows blank, not "0") and
// parsed to cents / ints on submit and for the live totals. Mirrors the offer
// editor exactly — the repair item shape is identical.
type ItemFormValue = {
  kind: string
  description: string
  quantity: string
...
function toFormValues(repair: RepairResponse): RepairFormValues {
  return {
    notes: repair.notes,
    taxRatePercent: String(repair.taxRateBps / 100),
    items: repair.items.map((it) => ({
      kind: it.kind,
...
type Props = {
  ...
  // The repair being edited. Editing is only offered while the repair is open.
  repair: RepairResponse
}

/**
 * The item editor for an OPEN repair. Line items are managed with useFieldArray;
 * useWatch drives a live totals preview that mirrors the server math. Once work
 * starts (in_progress) the repair is frozen server-side, so this dialog is only
 * opened for open repairs — a stale edit is refused with a 409.
 */
export function RepairFormDialog({ open, onOpenChange, repair }: Props) {
  const { t } = useLingui()
  const qc = useQueryClient()

  const values = useMemo(() => toFormValues(repair), [repair])
  const {
    register,
    control,
    handleSubmit,
  ...
  } = useForm<RepairFormValues>({ values })
  ...
  const bps = Math.round((parseFloat(watchedPercent) || 0) * 100)
  const tax = taxCents(subtotal, bps)
  const total = subtotal + tax

  const mutation = useMutation({
    mutationFn: (v: RepairFormValues) => {
      ...
      return api.repairs.update(repair.id, {
        notes: v.notes,
    ...
    // The server validates `items.<i>.unitPriceCents` and `taxRateBps`; the form
    // fields are `unitPrice` and `taxRatePercent`. Remap so each error lands on
    // the right input (same mapping as the offer editor).
    onError: (err) =>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            <Trans>Edit repair</Trans>
          </DialogTitle>
          <DialogDescription>
            <Trans>Line items and totals for this repair job.</Trans>
          </DialogDescription>
        </DialogHeader>
